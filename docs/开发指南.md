...
1. `profiles/mcp/<名称>.cordis.patch.yml`：MCP 的启动命令、环境变量。第三方 MCP 一律经 `scripts/mcp-readonly-proxy.mjs` 启动，用 `--allow` 列出放行的工具，其余工具对模型不可见、调用会被拒绝。
2. `scripts/common.mjs` 的 `featureRequirements`：需要的环境变量与命令。
...
5. `plugins/xunji-workbench/src/client/index.tsx` 的 `sourceForTool`：工具名到来源中文名的映射，用于回答尾部的来源标注。
6. 测试与文档。
