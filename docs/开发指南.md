...
构建需要两样工具：Rust（`https://rustup.rs`）与 Inno Setup（`winget install -e --id JRSoftware.InnoSetup`）。应用图标来自 `shell/icons/icon.ico`，安装包与界面内的品牌图都由它派生；换图后重新生成 `plugins/xunji-workbench/src/client/brand.ts` 并重新打包。
