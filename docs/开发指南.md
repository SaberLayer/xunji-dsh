...
面向想从源码运行、修改或打包寻迹助手的人。安装版的使用方式见 [使用说明](使用说明.md)。

## 当前组合

- DSH：固定 `0.1.5-rc.2`，不跟随 npm `latest`；原生右侧栏支持多标签、分栏、全屏与 Markdown / 代码 / PDF / 图片预览，对话可上传任意文件。
- 默认模型：`deepseek-flash`，支持图片输入。将 MasterGo 设计稿截图拖入对话即可进行视觉分析；Magic MCP 仍负责只读的 DSL/D2C 结构化数据。
- 寻迹助手：左侧栏“新会话”下方的“溯源配置”按钮打开中央工作台页（资料源配置与连接状态，不依赖会话）；会话中也可在原生右侧栏“+”里打开同名标签并排查看；每个回答尾部标注实际检索来源与依据。界面对外只出现“寻迹助手”；`xunji` 仅作为包名、CSS 类名和环境变量前缀等内部标识保留。
- Agent 预设：请使用“标准模式”。0.1.5 起“极简模式”只提供持久 Shell，并会屏蔽本项目的资料路由提示词与检索工具，`npm run doctor` 会对此提醒。
...
插件的构建产物 `plugins/xunji-workbench/lib/` 不入库：`npm test`、`npm run setup`、`npm run package` 都会先构建，改了插件源码直接跑这些命令即可。

## Windows 一键启动

首次按“快速开始”安装依赖后，直接双击项目根目录的 `启动 Xunji.cmd` 即可后台启动并自动打开浏览器；它会从启动日志解析带 `token` 的地址后再打开。它会复用已在运行的服务：改了代码想看效果，要先结束旧服务再启动。这是开发期的便捷入口，对外分发的安装版走的是 Tauri 桌面外壳，见下方“打包分发”。

## 按需启用资料源

先把 `.env.example` 复制为 `.env`，只填写需要的资料源。然后检查并启动：
...
每个资料源由几处登记组成，Confluence 与 MasterGo 的接入定义可作模板：

1. `profiles/mcp/<名称>.cordis.patch.yml`：MCP 的启动命令、环境变量。第三方 MCP 一律经 `scripts/mcp-readonly-proxy.mjs` 启动，用 `--allow` 列出放行的工具，其余工具对模型不可见、调用会被拒绝。自己实现的本机资料源用 `scripts/mcp-server.mjs` 的 `serveMcp` 提供协议骨架，只需写工具定义和 `call` 分发，参考 `scripts/codebase-mcp.mjs`。
2. `scripts/common.mjs` 的 `featureRequirements`：需要的环境变量与命令；开关型变量（如对话导入的 on / off）用 `enabled` 谓词声明何时算启用，不要在别处按名字特判。
3. `scripts/config-server.mjs` 的 `CONFIG_VARIABLES` 与 `validValue`：配置页允许保存的变量及校验。
4. `plugins/xunji-workbench/src/client/workflows.ts`：路由规则（什么问题该查它）、连接状态匹配、配置卡片与获取凭据步骤。
5. `plugins/xunji-workbench/src/client/sources.tsx` 的 `sourceForTool` 与 `evidenceFromResult`：工具名到来源中文名的映射，以及从结果里提取依据，用于回答尾部的来源标注。
6. 测试与文档。
...
产出 `dist/xunji-<版本>-setup.exe`，约 75MB。文件名只用 ASCII，因为 GitHub Release 会剥掉附件名里的非 ASCII 字符。双击安装包后可在向导里选择安装位置，默认装到用户目录，不需要管理员权限，也不需要预装 Node（运行时已内置）。首次启动会自动初始化，约一分钟，之后秒开。

打开的是独立桌面窗口，不是浏览器标签页：`shell/` 下的 Tauri 外壳负责窗口、启动编排与服务生命周期，界面用系统自带的 WebView2 渲染，外壳本身约 5MB。启动期间窗口内显示进度，失败时直接显示原因与日志路径；初始化偶发失败会自动重试一次。关闭窗口即结束后台服务；即使从任务管理器强制结束外壳，作业对象也会把服务进程树一并终止，不会留下占用端口的孤儿进程。

构建需要两样工具：Rust（`https://rustup.rs`）与 Inno Setup（`winget install -e --id JRSoftware.InnoSetup`）。正式发布不在本机做：把 `package.json` 的版本号改好后推送同名的 `v*` 标签，GitHub Actions 会在干净的 Windows 机器上完成全部构建并创建 Release，见 `.github/workflows/release.yml`。应用图标来自 `shell/icons/icon.ico`，安装包与界面内的品牌图都由它派生；换图后重新生成 `plugins/xunji-workbench/src/client/brand.ts` 并重新打包。
...
`npm run setup` 会把 DSH 版本写入对应 Profile 目录的 `xunji-dsh-version`；升级根依赖后、尚未对稳定 `.dsh/` 运行 `setup` 之前，`npm start` 会拒绝用新 CLI 启动旧 Profile，避免会话数据被提前单向迁移。切换步骤见 [upgrading.md](upgrading.md)。

## 安全原则

- `.env`、`.dsh/`、`.dsh-next/` 均已忽略，不提交凭据和会话数据。
- 外部资料源（MasterGo、飞书、Confluence）默认关闭，且凭据通过环境变量注入。
...
详细边界见 [architecture.md](architecture.md)，哪些数据会离开本机见 [隐私说明](隐私说明.md)。
