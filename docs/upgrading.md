...
6. 分别启用每个外部资料源的 overlay，确认工具清单、只读约束和错误降级。
...
能。项目只依赖 DSH 的 Profile、Bundle、MCP Client 和少量客户端插件接口。若客户端接口变化，通常只需升级 Workbench；外部资料源继续通过 MCP 隔离，知识数据库也不依赖 DSH 会话表结构。真正需要谨慎迁移的是 `.dsh` Profile 格式，因此始终先在 `.dsh-next` 验证。
